...
import { Github, Linkedin, MessageSquare, Copy, Check, Shield } from "lucide-react";
...
const ICON_MAP = { github: Github, linkedin: Linkedin, discord: MessageSquare, tryhackme: Shield };
    ...
    <section id="contact" className="py-28 px-6" aria-labelledby="contact-heading">
      <div className="max-w-5xl mx-auto">
        <RevealGroup className="mb-12">
          <RevealItem><p className="section-label mb-3">06 / Contact</p></RevealItem>
          <RevealItem>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          >
            ...
            {portfolio.contact.discord && (
              <div>
                ...
                <p className="t-small t-mono" style={{ color: "var(--text-muted)" }}>
                  {portfolio.contact.discord}
                </p>
              </div>
            )}
          ...
          >
            ...
            <div>
              ...
              <Input id="c-name" name="name" type="text" required value={state.name} onChange={onChange} placeholder="Your name" />
            </div>

            <div>
              <label htmlFor="c-email" className="block section-label mb-1.5">Email</label>
              <Input id="c-email" name="email" type="email" required value={state.email} onChange={onChange} placeholder="you@example.com" />
            </div>

            <div>
              <label htmlFor="c-message" className="block section-label mb-1.5">Message</label>
              <Textarea id="c-message" name="message" required rows={5} value={state.message} onChange={onChange} placeholder="What are you working on?" />
            </div>
            ...
            <button
              type="submit"
              disabled={sending}
              className="w-full py-3 rounded-xl t-small t-mono font-semibold transition-opacity hover:opacity-80 disabled:opacity-60 cursor-pointer"
              style={{ backgroundColor: "var(--accent)", color: "var(--accent-fg)" }}
            >
              {sending ? "$ sending..." : sent ? "$ message_sent ✓" : "$ send_message --secure"}
            </button>
          </motion.form>
        </div>
      </div>
